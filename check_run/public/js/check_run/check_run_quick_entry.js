...
frappe.ui.form.CheckRunQuickEntryForm = class CheckRunQuickEntryForm extends (
  ...
  default_accounts() {
    if (frappe.get_route() && frappe.get_route()[0] == "Form") {
      ...
      this.dialog.fields_dict["pay_to_account"].set_value(
        cur_frm.doc.pay_to_account
      );
      this.dialog.fields_dict["bank_account"].set_value(
        cur_frm.doc.bank_account
      );
    } else {
  ...
  register_primary_action() {
    ...
    this.dialog.set_primary_action(__("Start Check Run"), () => {
      ...
      frappe
        .xcall(
          "check_run.check_run.doctype.check_run.check_run.check_for_draft_check_run",
          {
            company: values.company,
            bank_account: values.bank_account,
            payable_account: values.pay_to_account,
          }
        )
        .then((r) => {

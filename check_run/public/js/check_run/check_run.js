...
check_run.mount_table = (frm) => {
  ...
  $(frm.fields_dict["check_run_table"].wrapper).html(
    $("<div id='check-run-vue'></div>").get(0)
  );
...
check_run.keyDownHandler = (e) => {
  ...
  if (
    e.keyCode == 40 &&
    check_run.frm.check_run_state.selectedRow <
      check_run.frm.check_run_state.transactions.length - 1
  ) {
    for (
      let j = 0;
      j < check_run.frm.check_run_state.transactions.length;
      j++
    ) {
    ...
    document
      .getElementById(`mop-input-${check_run.frm.check_run_state.selectedRow}`)
      .blur();

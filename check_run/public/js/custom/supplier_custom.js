frappe.ui.form.on('Supplier', {
	...
	supplier_default_mode_of_payment: frm => {
		set_required_banking_fields(frm)
	},
})
...
function show_bank_account_number(frm) {
	if (!frm.doc.bank || !frm.doc.bank_account) {
		...
		frappe
			.xcall('check_run.check_run.show_bank_account_number', { doctype: frm.doc.doctype, docname: frm.doc.name })
			.then(r => {
...
function set_required_banking_fields(frm) {
	if (!frm.doc.supplier_default_mode_of_payment) {
		return
	}
	frappe.db.get_value('Mode of Payment', frm.doc.supplier_default_mode_of_payment, 'type').then(r => {

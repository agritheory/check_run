...
@frappe.whitelist()
def gen_sepa_xml_file(data, company, pay_to_account):
    data = json.loads(data)
    posting_date = getdate()
    content = genrate_file_for_sepa(data, company, pay_to_account, posting_date)
    return content


def genrate_file_for_sepa(payments, company, pay_to_account, posting_date):
    ...
    content += make_line("          <PmtInfId>{0}</PmtInfId>".format(payments[0].get('payment_entry')))
    ...
    content += make_line("              <Id>")
    iban = get_iban_number(company, pay_to_account)
    ...
    for payment in payments:
        ...
        content += make_line("                  <InstrId>{}</InstrId>".format(payment.get('payment_entry')))
        content += make_line("                  <EndToEndId>{}</EndToEndId>".format(payment.get('payment_entry').replace('-',"")))
        content += make_line("              </PmtId>")
